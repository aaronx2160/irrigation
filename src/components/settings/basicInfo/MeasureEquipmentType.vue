  ...
  <div class="wrapper">
    ...
    >
      <el-form ref="form" label-width="auto">
        <el-form-item label="计量设施类型:">
          <el-input v-model.trim="MeasureTypeName" size="mini"></el-input>
        </el-form-item>
      </el-form>
    ...
    >
      <el-form ref="form" label-width="auto">
        <el-form-item label="计量设施类型:">
          <el-input v-model.trim="MeasureTypeEdit.name" size="mini"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      ...
      if (!this.MeasureTypeName) {
        this.$message.error('请输入计量设施类型名称！')
        return
      }
      http('post', '/api/basicInfo', {
        name: this.MeasureTypeName,
        type: this.type,
    ...
    handleEdit(row) {
      ...
      this.MeasureTypeEdit.id = row.Id
      this.MeasureTypeEdit.name = row.MeasureType.slice(0)
    },
    handleEditSubmit() {
      this.MeasureTypeEdit.type = this.type
      this.MeasureTypeEdit.colName = this.colName
      http('put', '/api/basicInfo', this.MeasureTypeEdit).then(() => {
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除该计量设施类型, 是否继续?', '提示', {

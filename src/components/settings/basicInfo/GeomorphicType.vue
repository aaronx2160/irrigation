<template>
  <div class="wrapper">
    ...
    <div>
      <template>
        <el-table :data="formData" style="width: 100%;" border stripe>
          <el-table-column label="地貌类型" prop="LandformType" align="center">
          </el-table-column>
    ...
    >
      <el-form ref="form" label-width="auto">
        <el-form-item label="地貌类型:">
          <el-input v-model.trim="LandformTypeName" size="mini"></el-input>
        </el-form-item>
      </el-form>
    ...
    >
      <el-form ref="form" label-width="auto">
        <el-form-item label="地貌类型:">
          <el-input v-model.trim="LandformTypeEdit.name" size="mini"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  data() {
    return {
      formData: [],
      LandformTypeName: '',
      colName: 'LandformType',
      type: 'syslandformtype',
      LandformTypeEdit: { id: '', name: '' },
  ...
  methods: {
    ...
    handleAdd() {
      ...
      if (!this.LandformTypeName) {
        this.$message.error('请输入地貌类型名称！')
        return
      }
      http('post', '/api/basicInfo', {
        name: this.LandformTypeName,
        type: this.type,
    ...
    handleEdit(row) {
      ...
      this.LandformTypeEdit.id = row.Id
      this.LandformTypeEdit.name = row.LandformType.slice(0)
    },
    handleEditSubmit() {
      this.LandformTypeEdit.type = this.type
      this.LandformTypeEdit.colName = this.colName
      http('put', '/api/basicInfo', this.LandformTypeEdit).then(() => {
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除该地貌类型, 是否继续?', '提示', {

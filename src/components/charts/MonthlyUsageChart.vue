...
export default {
    ...
    methods: {
        ...
        updateChart() {
            ...
            const areaNames = []
            this.allData.map((v) => {
                if (areaNames.indexOf(v.AreaName) === -1) {
                    areaNames.push(v.AreaName)
                }
            })
            for (let i = 0; i < areaNames.length; i++) {
                chartSeriesData[areaNames[i]] = [0, 0, 0]
            }
            for (let i = 0; i < this.allData.length; i++) {
                chartSeriesData[this.allData[i].AreaName][
        ...
        screenAdapter() {
            this.titleFontSize =
                (this.$refs.monthlyUsageRef.offsetWidth / 100) * 3.6
